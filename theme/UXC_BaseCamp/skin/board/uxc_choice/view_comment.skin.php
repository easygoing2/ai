...
if (!get_session('ss_token')) {
    set_session('ss_token', get_token());
}
?>
...
<section id="bo_vc" class="commentMedia">
    ...
    <?php
    // 채택된 댓글 ID 가져오기 (여분필드 wr_1 사용)
    $sql_parent = "SELECT wr_1 FROM {$write_table} WHERE wr_id = '{$view['wr_id']}' AND wr_is_comment = 0";
    $parent_post = sql_fetch($sql_parent);
    $chosen_comment_id = $parent_post['wr_1'] ?? '';
    
    // 채택된 댓글을 먼저 표시하기 위해 댓글 배열 재정렬
    $chosen_comment = null;
    $other_comments = array();
    
    $cmt_amt = count($list);
    for ($i=0; $i<$cmt_amt; $i++) {
        if ($chosen_comment_id && $list[$i]['wr_id'] == $chosen_comment_id) {
            $chosen_comment = $list[$i];
            $chosen_comment['_index'] = $i;
        } else {
            $other_comments[] = array_merge($list[$i], ['_index' => $i]);
        }
    }
    ...
    if ($chosen_comment) {
        $sorted_comments[] = $chosen_comment;
    }
    $sorted_comments = array_merge($sorted_comments, $other_comments);
    
    foreach ($sorted_comments as $idx => $comment_item) {
        $i = $comment_item['_index'];
        $list[$i] = $comment_item;
    ...
    <article id="c_<?php echo $comment_id ?>" class="media <?php echo ($member['mb_id']==$list[$i]['mb_id']) ? 'my' :''; ?>" <?php if ($cmt_depth) { ?>style="margin-left:<?php echo $cmt_depth ?>px;"<?php } ?>>
        <div class="media-body <?php echo ($chosen_comment_id && $comment_id == $chosen_comment_id) ? 'choice' : ''; ?>">
            ...
            <div class="media-content">
                ...
                <div class="cmt_contents">
                    <?php if ($chosen_comment_id && $comment_id == $chosen_comment_id) { ?>
                    <div class="chosen-badge" style="margin-bottom: 10px;">
                        <span class="button bg-pr color-wh-only round-s" style="padding: 5px 10px; font-size: 12px;">
                            <i class='bx bx-check-circle'></i> 채택된 답변
                        </span>
                    </div>
                    <?php } ?>
                    <p class="text">
                ...
                <div class="tools">
                    <?php 
                    // 글 작성자만 채택 버튼 표시, 채택된 댓글이 없을 때만 표시
                    if (!$chosen_comment_id && $view['mb_id'] && $view['mb_id'] == $member['mb_id'] && $list[$i]['mb_id'] != $view['mb_id']) { 
                    ?>
                        <button type="button" class="button bg-wh color-bl shadowline-de round-s sBtn bxicon btn-choose" onclick="chooseComment('<?php echo $comment_id ?>')">
    ...
    <?php if ($cmt_amt == 0) { //댓글이 없다면 ?><p id="bo_vc_empty">등록된 댓글이 없습니다.</p><?php } ?>
...
// 채택 기능 추가
function chooseComment(comment_id) {
    if (!confirm('이 댓글을 채택하시겠습니까?\n채택 후에는 변경할 수 없습니다.')) {
        return false;
    }
    
    $.ajax({
        url: '<?php echo $board_skin_url; ?>/choice_update.php',
        type: 'POST',
        data: {
