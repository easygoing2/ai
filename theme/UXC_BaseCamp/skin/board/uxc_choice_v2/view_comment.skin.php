...
<section id="bo_vc" class="commentMedia">
    ...
    <article id="c_<?php echo $comment_id ?>" class="media <?php echo ($member['mb_id']==$list[$i]['mb_id']) ? 'my' :''; ?>" <?php if ($cmt_depth) { ?>style="margin-left:<?php echo $cmt_depth ?>px;"<?php } ?>>
        <div class="media-body <?php echo (isset($write['wr_1']) && $write['wr_1'] == $comment_id) ? 'choice' : ''; ?>">
            ...
            <div class="media-content">
                ...
                <div class="cmt_contents">
                    <?php if (isset($write['wr_1']) && $write['wr_1'] == $comment_id) { ?>
                    <span class="button bg-pr color-wh-only round-s bxicon sBtn"><i class='bx bxs-badge-check'></i> 채택된 답변</span>
                    <?php } ?>
                    <p class="text">
                ...
                <?php
                // 채택 가능 여부: 글 작성자 본인 + 아직 미채택 + 타인의 댓글 + 회원 댓글
                $is_choice_target = ($is_member && $member['mb_id'] == $write['mb_id']
                    && (!isset($write['wr_1']) || !$write['wr_1'] || $write['wr_1'] == '0')
                    && $list[$i]['mb_id'] && $list[$i]['mb_id'] != $member['mb_id']);
                ?>
                <?php if($is_comment_reply_edit || $is_choice_target) { ?>
                <div class="tools">
                    ...
                    <?php } ?>
                    <?php if ($is_choice_target) { ?>
                        <button type="button" class="button bg-pr color-wh-only round-s bxicon sBtn btn-choice" data-comment-id="<?php echo $comment_id; ?>" title="채택">
                            <i class='bx bxs-badge-check'></i> 채택하기
                        </button>
                    <?php } ?>
...
$(function() {
    ...
    // 채택 버튼 (이벤트 위임 - AJAX 댓글 교체 후에도 동작)
