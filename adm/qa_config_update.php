...
$check_keys = array('qa_title', 'qa_category', 'qa_skin', 'qa_mobile_skin', 'qa_use_email', 'qa_req_email', 'qa_use_hp', 'qa_req_hp', 'qa_use_sms', 'qa_send_number', 'qa_admin_hp', 'qa_admin_email', 'qa_use_editor', 'qa_select_editor', 'qa_subject_len', 'qa_mobile_subject_len', 'qa_page_rows', 'qa_mobile_page_rows', 'qa_image_width', 'qa_upload_size');

foreach ($check_keys as $key) {
    $$key = $_POST[$key] = isset($_POST[$key]) ? strip_tags(clean_xss_attributes($_POST[$key])) : '';
}

if ($qa_select_editor && (!preg_match('/^[a-zA-Z0-9_-]+$/', $qa_select_editor) || !is_file(G5_EDITOR_PATH . '/' . $qa_select_editor . '/editor.lib.php'))) {
    $qa_select_editor = $_POST['qa_select_editor'] = '';
}
