...
<section class="wzc-calendar" id="wzcCalendar" aria-labelledby="wzcCalendarTitle">
  <header class="wzc-heading">
    <div>
      ...
      <h2 id="wzcCalendarTitle">학습진행 캘린더</h2>
      <p>캘린더로 시청한 강의들을 확인하고 학습 일정을 관리할 수 있습니다. 그리고 개인일정도 등록해서 관리할 수 있습니다.</p>
    </div>
    <div class="wzc-heading-actions">
      <button type="button" class="wzc-btn wzc-btn-ghost" data-open-modal="settings"><i
          class="bx bx-slider-alt"></i><span>설정</span></button>
      <button type="button" class="wzc-btn wzc-btn-primary"
        data-add-event="<?php echo htmlspecialchars($selected_day, ENT_QUOTES, 'UTF-8'); ?>"><i
          class="bx bx-plus"></i><span>일정 추가</span></button>
    </div>
  </header>

  <div class="wzc-toolbar">
    <a class="wzc-nav-button" href="<?php echo $month_url($calendar->prev_year, $calendar->prev_month); ?>"
      aria-label="이전 달"><i class="bx bx-chevron-left"></i></a>
    <strong class="wzc-month-title"><?php echo (int)$calendar->year; ?>년
      <?php echo htmlspecialchars($calendar->month_text, ENT_QUOTES, 'UTF-8'); ?>월</strong>
    <a class="wzc-nav-button" href="<?php echo $month_url($calendar->next_year, $calendar->next_month); ?>"
      aria-label="다음 달"><i class="bx bx-chevron-right"></i></a>
    <a class="wzc-today-button"
      href="<?php echo $month_url((int)substr($calendar->today, 0, 4), (int)substr($calendar->today, 5, 2), $calendar->today); ?>">오늘</a>
  </div>
  ...
  <div class="wzc-filter" aria-label="일정 분류 필터">
    ...
    <button type="button" class="wzc-filter-chip" data-category-filter="<?php echo (int)$category['wc_ix']; ?>">
      <span class="wzc-color-dot"
        style="--wzc-category-color:<?php echo htmlspecialchars($category['wc_color'], ENT_QUOTES, 'UTF-8'); ?>"></span>
  ...
  <div class="wzc-weekdays" aria-hidden="true">
    <span class="is-sunday">일</span><span>월</span><span>화</span><span>수</span><span>목</span><span>금</span><span
      class="is-saturday">토</span>
  </div>

  <div class="wzc-grid" role="grid"
    aria-label="<?php echo (int)$calendar->year; ?>년 <?php echo (int)$calendar->month; ?>월 개인 일정">
    <?php foreach ($calendar->cells as $cell) {
        ...
        ?>
    <section class="<?php echo implode(' ', $classes); ?>" role="gridcell"
      data-date="<?php echo htmlspecialchars($cell['date'], ENT_QUOTES, 'UTF-8'); ?>">
      <div class="wzc-day-header">
        <a href="<?php echo $month_url($cell_year, $cell_month, $cell['date']); ?>" class="wzc-day-number"
          aria-label="<?php echo htmlspecialchars($cell['date'], ENT_QUOTES, 'UTF-8'); ?> 일정 보기"><?php echo (int)$cell['day']; ?></a>
        <?php if ($cell['today']) { ?><span class="wzc-today-label">오늘</span><?php } ?>
        <button type="button" class="wzc-day-add"
          data-add-event="<?php echo htmlspecialchars($cell['date'], ENT_QUOTES, 'UTF-8'); ?>"
          aria-label="<?php echo htmlspecialchars($cell['date'], ENT_QUOTES, 'UTF-8'); ?> 일정 추가"><i
  ...
  <section class="wzc-day-agenda" aria-labelledby="wzcAgendaTitle">
    <div class="wzc-agenda-heading">
      ...
      <button type="button" class="wzc-btn wzc-btn-primary wzc-btn-small"
        data-add-event="<?php echo htmlspecialchars($selected_day, ENT_QUOTES, 'UTF-8'); ?>"><i class="bx bx-plus"></i>
        일정 추가</button>
    </div>
    <div class="wzc-agenda-list">
      ...
      <div class="wzc-empty"><i class="bx bx-calendar"></i>
        <p>등록된 일정이 없습니다.</p>
      </div>
      <?php } else { foreach ($selected_events as $event) { $data = wzc_event_public_data($event); ?>
      <button type="button" class="wzc-agenda-item wzc-event-open" data-event-id="<?php echo (int)$data['id']; ?>"
        style="--wzc-event-color:<?php echo htmlspecialchars($data['color'], ENT_QUOTES, 'UTF-8'); ?>">
        <span
          class="wzc-agenda-time"><?php echo $data['all_day'] ? '종일' : htmlspecialchars($data['start_time'], ENT_QUOTES, 'UTF-8'); ?></span>
        <span
          class="wzc-agenda-main"><strong><?php echo htmlspecialchars($data['title'], ENT_QUOTES, 'UTF-8'); ?></strong><small><?php echo htmlspecialchars($data['category_name'] ?: '분류 없음', ENT_QUOTES, 'UTF-8'); ?><?php echo $data['location'] ? ' · '.htmlspecialchars($data['location'], ENT_QUOTES, 'UTF-8') : ''; ?></small></span>
    ...
    <form id="wzcEventForm" class="wzc-form">
      ...
      <div class="wzc-field">
        ...
        <select name="category_id" id="wzcCategory">
          ...
          <?php foreach ($categories as $category) { ?><option value="<?php echo (int)$category['wc_ix']; ?>"
            <?php echo $default_category === (int)$category['wc_ix'] ? 'selected' : ''; ?>>
            <?php echo htmlspecialchars($category['wc_name'], ENT_QUOTES, 'UTF-8'); ?></option><?php } ?>
        </select>
      </div>
      ...
      <div class="wzc-order-actions" id="wzcOrderActions" hidden aria-label="일정 순서 변경">
        <span>선택 날짜 순서</span>
        <button type="button" class="wzc-btn wzc-btn-ghost wzc-btn-small" data-order-direction="up"><i
            class="bx bx-up-arrow-alt"></i> 위로</button>
        <button type="button" class="wzc-btn wzc-btn-ghost wzc-btn-small" data-order-direction="down"><i
            class="bx bx-down-arrow-alt"></i> 아래로</button>
        <button type="button" class="wzc-btn wzc-btn-ghost wzc-btn-small" data-order-direction="top"><i
            class="bx bx-chevrons-up"></i> 맨 위로</button>
      </div>
...
<div class="wzc-modal" id="wzcSettingsModal" aria-hidden="true">
  <div class="wzc-modal-backdrop" data-close-modal></div>
  <section class="wzc-modal-dialog wzc-settings-dialog" role="dialog" aria-modal="true"
    aria-labelledby="wzcSettingsTitle">
    <header class="wzc-modal-header">
        ...
        <div class="wzc-category-list">
          ...
          <form class="wzc-category-form" data-category-id="<?php echo (int)$category['wc_ix']; ?>">
            <input type="color" name="color"
              value="<?php echo htmlspecialchars($category['wc_color'], ENT_QUOTES, 'UTF-8'); ?>" aria-label="분류 색상">
            <input type="text" name="name"
              value="<?php echo htmlspecialchars($category['wc_name'], ENT_QUOTES, 'UTF-8'); ?>" maxlength="50"
              aria-label="분류명">
            <button type="submit" class="wzc-icon-btn" aria-label="분류 저장"><i class="bx bx-check"></i></button>
            <button type="button" class="wzc-icon-btn is-danger" data-delete-category aria-label="분류 삭제"><i
        ...
        <form id="wzcPreferenceForm" class="wzc-preference-form">
          ...
          <label>날짜 셀 일정 표시
            <select name="events_per_day">
              <?php for ($i = 1; $i <= 10; $i++) { ?><option value="<?php echo $i; ?>"
                <?php echo $events_per_day === $i ? 'selected' : ''; ?>>최대 <?php echo $i; ?>개</option><?php } ?>
            </select>
          </label>
          <label class="wzc-switch-row"><span>모바일 드래그 사용</span><input type="checkbox" name="touch_drag_use" value="1"
              <?php echo !empty($preference['wp_touch_drag_use']) ? 'checked' : ''; ?>></label>
...
<script type="application/json" id="wzcRuntimeConfig">
