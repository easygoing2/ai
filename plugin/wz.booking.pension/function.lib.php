...
function wz_check_date_room($rm_ix, $fr_date) { 
    ...
    for ($i=0; $i<$max_day; $i++) { 
        ...
        $rms = sql_fetch($query);
        if (!empty($rms['rms_ix'])) { // 예약이 불가능한 날짜라면.
            $max_day = $i;
            break;
        }
    } 
...
function wz_calculate_room($parm) { 
    ...
    $cnt_chk = 0;
    if (isset($parm['chk']) && is_array($parm['chk'])) {
        $cnt_chk = count($parm['chk']);
        for ($z = 0; $z < $cnt_chk; $z++) {
            $idx    = (int)$parm['chk'][$z];
            $rmix   = isset($parm['rm_ix'][$idx]) ? (int)$parm['rm_ix'][$idx] : 0; // 객실키
            ...
            if ($rmix) { 
                ...
                $bkday          = isset($parm['bk_day'][$idx]) ? (int)$parm['bk_day'][$idx] : 0; // 예약일수
                if ($bkday < 1) {
                    continue;
                }
                $price_room     = 0;
                for ($j=0;$j<$bkday;$j++) { 
                    $rms_date   = wz_get_addday(isset($parm['sch_day']) ? $parm['sch_day'] : '', $j);
                ...
                $person_min     = isset($parm['person_min'][$idx]) ? (int)$parm['person_min'][$idx] : 0; // 최소인원
                $person_max     = isset($parm['person_max'][$idx]) ? (int)$parm['person_max'][$idx] : 0; // 최대인원
                $cnt_adult      = isset($parm['bk_cnt_adult'][$idx]) ? (int)$parm['bk_cnt_adult'][$idx] : 0; // 선택인원 - 성인
