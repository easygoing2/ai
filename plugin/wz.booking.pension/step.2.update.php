...
$bk_hp3             = isset($_POST['bk_hp3'])           ? trim($_POST['bk_hp3'])                : "";
...
if (!$cnt_room) {
    alert('선택한 객실정보가 없습니다.', WZP_STATUS_URL);
}
...
if ($cnt_room > 0) { 
    for ($z = 0; $z < $cnt_room; $z++) { 
        ...
        for ($j=0;$j<$bk_day;$j++) { 
            ...
            $rms = sql_fetch($query);
            $rms_status = isset($rms['rms_status']) ? $rms['rms_status'] : '';
            if ($rms_status == '완료' || $rms_status == '예약완료') { // 이미 예약중인 날짜라면.
                $error_msg .= '\"'.$arr_room[$z]['rm_subject'].'\" 의 '.wz_get_hangul_date($rms_date).' 예약이 이미 완료된 예약객실로 예약이 불가능합니다.\\n잔여객실 확인 후 다시 예약바랍니다.\\n';
                $error++;
            }
            else if ($rms_status == '대기' || $rms_status == '예약대기') { // 이미 예약중인 날짜라면.
